...
class MTrans(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.evi_uncertainty:
            ...
            box_uncertainty = boxes[1:]
            box = boxes[0]                                                                              # B x 7
            pred_dict['box_uncertainty'] = box_uncertainty 
    ...
    def get_loss(self, pred_dict, data_dict, rank, weights=None):
        ...
        if self.laplace_uncertainty:
            ...
            l_loc0 =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_loc[:,0], pred_loc[:,0]) + log_variance).mean()
            l_loc1 =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_loc[:,1], pred_loc[:,1]) + log_variance).mean()
            l_loc2 =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_loc[:,2], pred_loc[:,2]) + log_variance).mean()
            l_dim0 =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_dim[:,0], pred_dim[:,0]) + log_variance).mean()
            l_dim1 =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_dim[:,1], pred_dim[:,1]) + log_variance).mean()
            l_dim2 =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_dim[:,2], pred_dim[:,2]) + log_variance).mean()
            l_yaw =  (1.4142 * torch.exp(-log_variance) * criterion_lapl(gt_yaw, pred_yaw) + log_variance).mean()
            # l_iou =  1.4142 * torch.exp(-log_variance) * l_iou + log_variance
            loss_dict['lapl_unc_checker'] = ((torch.exp(log_variance) - iou3d).abs().mean().item(), num_gt_samples, 'iou')
        ...
        if self.evi_uncertainty:
            evi_lambda = 1
            evidential_loss = evidential_regression_loss(gt_boxes, (pred_boxes,) + pred_dict['box_uncertainty'], evi_lambda)
