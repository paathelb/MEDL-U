...
def kitti_collate_wrapper(batch:list):
    ...
    weights = [obj['weight'] for obj in batch]
...
def build_kitti_loader(dataset, cfg, loader_config, rank, num_gpus, labeled=True):
    # Fix generator for reproducibility
    g = torch.Generator()
    g.manual_seed(loader_config.random_seed)

    if cfg.dist and loader_config.shuffle==True:
        sampler = torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=num_gpus, rank=rank)    # modified/changed by Helbert PAAT for multi-gpu training
    else:
        sampler = None
    ...
    if cfg.dist:
        loader = DataLoader(dataset, 
                            ...
                            num_workers=loader_config.num_workers, # 4*num_gpus, # changed-modified by Helbert PAAT
    ...
    # Only applies for unlabeled dataset during the run.predict function but not during training
    elif not labeled:
        loader = DataLoader(dataset, 
                            batch_size=loader_config.batch_size,
                            num_workers=loader_config.num_workers, # 4*num_gpus, # changed/modified by Helbert PAAT
                            shuffle = loader_config.shuffle_for_unlabeled,
                            collate_fn=kitti_collate_wrapper,
                            pin_memory=loader_config.pin_memory,
                            drop_last=loader_config.drop_last,
                            generator=g,
...
def move_to_cuda(data_dict, device, rank):
    for k in data_dict.keys():
        if isinstance(data_dict[k], torch.Tensor):
            data_dict[k] = data_dict[k].cuda(rank, non_blocking=True)
    return data_dict

def make_tensor_keys(data_dict):
