...
from easydict import EasyDicts
...
class MTrans(nn.Module):
    ...
    def get_loss(self, pred_dict, data_dict, rank, weights=None):
        ...
        if self.evi_uncertainty:
            ...
            S = alpha.sum(dim=1).unsqueeze(1).repeat(1,2,1)        # B x N
            ...
            # Loss based on the most uncertain points 
            seg_uncer = C / alpha
            topk = 10
            topk_indices = torch.topk(seg_uncer, topk, dim=1).indices
            segment_logits_topk = torch.gather(segment_logits.permute(0,2,1), 2, topk_indices.unsqueeze(-1))
            gt_segment_label_topk = torch.gather(F.one_hot(gt_segment_label, num_classes=2), 2, topk_indices.unsqueeze(-1))
            L_un = torch.sqrt(torch.sum(torch.square(gt_segment_label_topk - segment_logits_topk)))
