...
def img2pc(feature_img, points_2d):
    ...
    points_2d = points_2d.view(-1, 2)                                   # (B*N) x 2
    x, y = points_2d[:, 0], points_2d[:, 1]                             # B*N
    # query corresponding img feature with the 2D coordinates
    new_pc_features = torch.zeros(B*N, C_img, device=feature_img.device) # (B*N) x 512
    total_weight = 0
    for ty in [torch.ceil(y), torch.floor(y)]:
        for tx in [torch.ceil(x), torch.floor(x)]:
            weight = (1-torch.abs(x-tx)) * (1-torch.abs(y-ty))          # B*N
            temp_feature_img = feature_img[batch_idx, :, ty.long().clamp(max=H-1), tx.long().clamp(max=W-1)]  # B*N x 512
            new_pc_features = new_pc_features + weight.unsqueeze(1) * temp_feature_img                        # B*N x 512 
            # TODO Why not just use combination that can give bigger weight? TODO Why not just use rounded off number for ty and tx and not weighted combination?
            total_weight = total_weight + weight.unsqueeze(1)                                                 # B*N x 1 # TODO Intuition with adding weight?
    new_pc_features = new_pc_features / total_weight # B*N x 512 # TODO Intuition? total_weight is always equal to 1
    new_pc_features = new_pc_features.view(B, N, C_img).transpose(-1, -2) # B x C_img x N
