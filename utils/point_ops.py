...
def build_image_location_map_single(H, W, device='cuda'):
    locations = torch.zeros(2, H, W, dtype=torch.float32, device=device)    # 2 x H x W
    row_idx = torch.arange(H, dtype=torch.long, device=device) # H # Values from 0 to H-1
    col_idx = torch.arange(W, dtype=torch.long, device=device) # W # Values from 0 to W-1
    locations[1, row_idx, :] = row_idx.view(1, -1, 1).float() # 2 x H x W
    locations[0, :, col_idx] = col_idx.view(1, 1, -1).float() # 2 x H x W
    locations = locations.permute(1, 2, 0)  # H, W, 2 for (x, y) 2D coords
    return locations # H x W x 2
